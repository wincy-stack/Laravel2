                    ...
                    <form action="{{ route('order.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('rice_id') is-invalid @enderror" 
                                ...
                                @foreach($rices as $rice)
                                    <option value="{{ $rice->id }}" data-price="{{ $rice->price_per_kg }}" data-stock="{{ $rice->stock }}">
                                        {{ $rice->name }} (Stock: {{ $rice->stock }} kg) - ₱{{ number_format($rice->price_per_kg, 2) }}/kg
                                    </option>
                                @endforeach
                            </select>
                            @error('rice_id')
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" class="form-control @error('quantity') is-invalid @enderror" 
                                       ...
                                       placeholder="0.00" step="0.1" min="0.1" oninput="calculateTotal()" required>
                                <span class="input-group-text">kg</span>
                            </div>
                            <small class="text-muted">Maximum available: <span id="maxStock">-</span> kg</small>
                            @error('quantity')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="card bg-light mb-3">
                            <div class="card-body">
                                ...
                                <h4 class="text-primary">₱<span id="pricePerKg">0.00</span></h4>

                                <hr>

                                <h6 class="text-muted">Total Amount</h6>
                                <h3 class="text-success">₱<span id="totalAmount">0.00</span></h3>
                            </div>
                        </div>
